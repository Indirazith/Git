...
public class ScoreBoard {
	 ...
	 * @param target
	 */
	void inningsDetails(String player1, String player2, int target) {
	...
	public void finalScore(Innings firstInning, Innings secondInnings, int target) {
		if (secondInnings.getTotalScore() >= target) {
			System.out
					.println(secondInnings.getBatsMan() + " wins with " + secondInnings.getBalls() + " balls to spare");
		} else {
			System.out.println(secondInnings.getBowler() + " wins by " + firstInning.getTotalScore() + " runs ");
		}
	}
